...
return function (uri, callback)
    ...
    guide.eachSourceType(state.ast, 'call', function (source)
        ...
        if isLocalMethod(source, 'wf', 'task') then
            local spec = source.args and source.args[2]
            if not spec or spec.type == 'nil' then
                callback {
                    start   = source.start,
                    finish  = source.finish,
                    message = "devsper: wf.task() requires a spec table as second argument",
                }
                return  -- no further checks possible
            end
            if spec.type ~= 'table' then return end  -- non-literal, skip
        ...
        elseif isLocalMethod(source, 'wf', 'plugin') then
            local spec = source.args and source.args[2]
            if not spec or spec.type == 'nil' then
                callback {
                    start   = source.start,
                    finish  = source.finish,
                    message = "devsper: wf.plugin() requires a spec table as second argument",
                }
                return  -- no further checks possible
            end
            if spec.type ~= 'table' then return end  -- non-literal, skip
        ...
        elseif isDevsperGlobalMethod(source, 'tool') then
            ...
            if not spec or spec.type == 'nil' then
                callback {
                    start   = source.start,
                    finish  = source.finish,
                    message = "devsper: devsper.tool() requires a spec table as second argument",
                }
                return  -- no further checks possible
            end
            if spec.type ~= 'table' then return end  -- non-literal, skip
