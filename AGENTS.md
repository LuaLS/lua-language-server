...
- 里程碑 2 进行中：已迁移 30 条规则（parser-only + VM 语义：`undefined-field`/`undefined-global`/`deprecated`/`need-check-nil`/`redundant-parameter`/`missing-parameter`/`assign-type-mismatch`/`param-type-mismatch`/`return-type-mismatch`/`global-in-nil-env`）。
