...
- **类型差集运算符 `~`**（cat 层 `A ~ B`，与 `|`/`&` 对应；注意 Lua **表达式**层的 `~` 仍是按位异或，互不干扰）：优先级高于 `&`（union → subtract → intersection → term）。`Node.Subtract`（`script/node/subtract.lua`，`ls.node.kind` 已登记 `subtract` 位）是静态差集求值器：`value` getter 按 A 的 union 成员做 `m:canCast(B)` 排除，**Generic 成员保留**（未实例化时"⊆ B"不可判，且 `ANY canCast 任意` 的 cast 允许语义会误判为包含）；`resolveGeneric(map, ctx)` 用 resolve 后的 a/b **重建** Subtract（泛型先例化再求差集，避免求值时机错位）；`rt.subtract(a, b)` 特判 `a~never=never`、`b=any→a`。meta 用法：`ipairs`/`pairs` 签名已改为 `fun<V>(...):integer, V ~ nil` / `fun<K, V>(...):K, V ~ nil`（迭代器第 2 返回去掉 nil——for-in 循环体内非 nil）。
...
- `return-type-mismatch`：函数 return 类型不匹配。`ret.parent` 是 function → `vfile:getNode(parent)` 得 `Node.Function` → `f:getReturn(i)` 比较。**注意无注解函数跳过**（`#f.returnsDef == 0` 才不查，否则 `getReturn` 会从 returnList 推断出 literal Value，与转 type 后的 actual canCast 失败导致误报）。expect 是 any/unknown 跳过；actual 不做 any 豁免（用户否决）。**任意 key 表 cast 数组/元组的宽进**：`{ [unknown]: X }`（`Node.Table:isAnyKeyTable`——存在 unknown/any 类型 key 字段，来自动态 index 赋值如 `objs[i] = v`）cast `Array` 或 `Tuple`（注解 `[T]` 编译为 **Tuple** 而非 Array）时，元素比较 `X canCast T` 失败后退回 `T canCast X`（值域覆盖元素要求，`{[unknown]: X}` 视为 `[X]`）；`Tuple:onCanBeCast` 对 anyKeyTable 用双向元素比较。具名 key 表（如 `{[1]: X}`）仍走严格单向。
- `missing-return-value`：函数缺少必需返回值。**不能用 `returnsPack.min`**（与 `paramsPack.min` 同坑：`function.lua` 的 `returnsPack.min` 不排除 optional 返回）。正确做法遍历 `funcNode.returnsDef` 数 `not r.optional` 的数量；遇到 `r.value` 是 spread 或 `typeName=='...'`（变参返回）则不限 min，直接跳过不报。可变长返回（`---@return ...`）同跳过。`r.optional` 来源：`---@return A? name` 的 `?` 标在 CatStateReturnItem 的 **value 节点** 上（`value.optional`，parser `parseCatStateReturnItem`），由 function coder `addReturnDef` 第三参传入（曾漏传导致 `A, B, C?` 报必须 3 返回值）；`fun(): A?` 类型路径走 cat.lua catfunction coder 本就传 optional。
...
- `await-in-sync`：同步函数内调用 async 函数。默认 `status='None'`（group `await`）。上下文判定：沿 `call.parent` 上溯首个 `isFunction` 节点（`isMain` 视为 async 上下文直接跳过），`vfile:getNode(func).async` 为真则跳过。callee 判定：`vfile:getVariable(call.node):hasAnnotation('async')`（裸 `---@async` 经 `tryBindCat` 绑变量注解）或 `vfile:getNode(call.node)` 经 `each('function')` 任一 `Node.Function.async`（`---@async` 经 function coder `setAsync`、`async fun()` 类型经 catfunction coder `setAsync`）。行内 `function () ---@async` 已支持：parser 侧 `blockStart` 已移到块开启符后首个 `skipSpace` 之前（while/do/if×3/repeat/function；for 原本就对），使 `do`/`then`/`else`/`repeat`/函数头后同行及下一行首的注释进入该块自身 delayComments（此前落入外层 block，导致 while 体内 `---@async` 绑不上语句——work.lua 误报根因）；coder 侧 function coder 在函数体编译后扫描自身 childs 中 `finishRow == startRow` 的行内 async cat 补 `setAsync`。不支持：`Lua.hint.awaitPropagate` 传播、master 的 linked-call（async 回调参数被调内视为 async 调用）。
