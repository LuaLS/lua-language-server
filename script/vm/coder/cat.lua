...
ls.vm.registerCoderProvider('catid', function (coder, source)
    ...
    elseif source.genericTemplate then
        local bad
        local parts = ls.util.replaceInside(source.id, '`', '`', function (content, inside)
            if inside then
                local generic = source.genericTemplate[content]
                if generic then
                    return coder:getKey(generic)
                else
                    bad = true
                end
            end
        ...
        if bad then
            coder:addLine('{id} = rt.UNKNOWN' % {
                id = coder:getKey(source),
            })
        else
            coder:addLine('{id} = rt.oddTemplate { {parts} }' % {
                id    = coder:getKey(source),
                name  = source.id,
...
ls.vm.registerCoderProvider('catblock', function (coder, source)
    ...
    coder:addLine('---@diagnostic disable')
    coder:addLine('local coder, vfile, rt, r = nil')
    coder:addLine('local uri = uri')
    coder:addLine('---@diagnostic enable')
